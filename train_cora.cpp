...
void train(Graph<T>& g, int hidden, int epochs, T lr, T wd, bool use_adam) {
    ...
    printf("optimizer=%s  lr=%g  wd=%g\n", use_adam ? "AdamW" : "SGD", lr, wd);
    printf("epoch | train_loss | train_acc | val_acc\n");
    for (int e = 0; e < epochs; ++e) {
        DenseMatrix<T> logits = L2.forward(L1.forward(g.X));
        ...
        L1.backward(L2.backward(dlogits));
        ...
        if (e % 20 == 0 || e == epochs - 1) {
            double tr = accuracy_masked(logits, g.labels, g.train_mask);
            double va = accuracy_masked(logits, g.labels, g.val_mask);
            printf("%5d | %10.4f | %8.3f | %7.3f\n", e, loss, tr, va);
        }
    }
    DenseMatrix<T> logits = L2.forward(L1.forward(g.X));
    printf("\n==> final TEST accuracy = %.4f\n", accuracy_masked(logits, g.labels, g.test_mask));
}

int main(int argc, char** argv) {
    seed_rng(42);
    // 解析参数：cora 文件 / 优化器选择(sgd|adam)
    ...
    // Cora 推荐：AdamW lr=0.01 wd=5e-4 hidden=16
    train(g, /*hidden=*/16, /*epochs=*/200, /*lr=*/use_adam ? T(0.01) : T(0.2),
          /*wd=*/use_adam ? T(5e-4) : T(0), use_adam);
    return 0;
}
